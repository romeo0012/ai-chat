...
class RAGEngine {
  constructor(dataDir) {
    ...
    this.loaded = false
    this.SOURCES = [
      { host: 'www.virtuozzo.com', header: 'Dokumentace PaaS T Business Cloud:' },
      { host: 'docs.cloudsigma.com', header: 'docs.cloudsigma.com (CloudSigma — IaaS Dev Docs):' },
  ...
  searchPerSource(query, topKPerSource = 3, maxSources = 8, opts = {}) {
    ...
    for (const source of this.SOURCES) {
      // Skip sources the client did not select — don't even score their chunks.
      if (opts.hosts && opts.hosts.length && !opts.hosts.includes(source.host)) continue
      const idxs = []
      for (let i = 0; i < docCount; i++) {
  ...
  formatSourceContext(groups, opts = {}) {
    ...
    // Strip rendering/translation artifacts out of the retrieved doc text so the
    // model doesn't copy them: code line-number runs, draft blocks, hash alts.
    const cleanDocText = (text) => text
      .split('\n')
      .map(line => line.trim())
      .filter(line => {
        if (!line) return false
        if (/^(?:\d+\s*){3,}$/.test(line)) return false
        if (/^(?:překlad\s+(?:po\s+úpravách|po\s+úpravě|prozatím(?:\s+(?:ne\s+)?prozatímně)?)|text\s+v\s+češtině):/i.test(line)) return false
        if (/(?:hu)?[0-9a-f]{32}\s+\d+\s+\d+x\d+(?:\s*(?:resize\s+)?box\s+\d+)?/.test(line)) return false
        return true
      })
      .join('\n')
      .replace(/(^|\n)\d+( \d+){4,}(\n|$)/g, '$1$3')
    ...
    for (const source of this.SOURCES) {
      ...
      if (g && g.results.length > 0) {
        for (const r of g.results) {
          const content = cleanDocText(r.content || '').slice(0, charsPerDoc) || '(žádné dokumenty pro tento dotaz)'
