...
class LLMClient {
  ...
  async streamResponse(body, headers, onStream) {
    ...
    try {
      ...
      // Attach the real rejection reason from the endpoint for diagnostics
      // (e.g. vLLM 400 "This model's maximum context length is N tokens...").
      // For streaming requests the body is a socket/stream, so read it first.
      try {
        let detail = ''
        const data = err.response && err.response.data
        if (typeof data === 'string') detail = data
        else if (Buffer.isBuffer(data)) detail = data.toString('utf8')
        ...
        if (detail) err.message = `${err.message} :: ${detail.slice(0, 600).replace(/\s+/g, ' ')}`
      } catch {}
      throw err
    }
